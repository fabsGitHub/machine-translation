...
from utils import check_artifact_cache, is_cache_valid, set_seed
...
class AsyncEvaluationQueue:
    ...
    def sync_study(self):
        ...
        if self.futures:
            ...
            self.futures.clear()
            print(
                "🎯 [Study Barrier Cleared] All study models evaluated successfully.\n"
            )
...
def print_study_model_and_batch_info(
    ...
    print(
        f" ├─ Architecture:             {rnn_type} (BiDirect: {bidi_bool}, Attention: {attention_type})"
    )
    print(f" ├─ Dimensions:               Emb={emb_dim} | Hidden={hidden_dim}")
    print(f" ├─ Batch Size (N samples):   {batch_num} sequences / batch")
    print(
        f" ├─ Batch Shape Estimate:     [{batch_num}, {seq_len}] ({batch_memory_mb:.4f} MB per batch tensor)"
    )
...
def run_cmd(args_list):
    ...
    if "--tf_ratio" not in args_list:
        args_list = ["--tf_ratio", "0.4"] + args_list

    # Deduplicate and clean argument lists
    i = 0
    kv = {}
    positional = []
    ...
    # OPTIMIZATION: Bypass torch.distributed.run when on single GPU to eliminate IPC overhead
    if nproc > 1:
        command = [
            sys.executable,
            "-m",
    ...
    print(
        f"\n🚀 Launching Execution Unit ({'DDP ' + str(nproc) + ' GPUs' if nproc > 1 else 'Single-GPU Direct'}): {' '.join(command)}"
    )
    ...
    print(
        f"⏱️ Done. Duration: {duration:.2f}s | Avg/Epoch: {duration/max(1, epochs):.2f}s"
    )
...
def run_auto_evaluation(experiment_id, rnn_type):
    ...
    if os.path.exists(target_model):
        ...
        try:
            ...
            print(
                f"⚠️ Process protection enabled for {experiment_id}. Processing via fallback structures."
            )
...
def sync_ledger_to_token_type(token_type):
    ...
    try:
        ...
        for k, v in g_data.items():
            if k.upper().startswith(token_type.upper()):
                ...
                study_ledger_path = os.path.join(
                    REPO_ROOT,
                    f"evaluation_ledger_{token_type}_{study_suffix}.json",
                )
        ...
        if remaining_g_data:
            with open(global_ledger, "w") as f:
...
def get_best_hyperparameters(stage, token_type, rnn_type=None):
    ...
    if not os.path.exists(csv_path):
        ...
        if tune_configs:
            ...
            if best_cfg:
                ...
                print(
                    f"🎯 Optimization Checkpoint Found from JSON files! Applying Tuned Parameters: --lr {lr} --dropout {dropout} --emb_dim {emb_dim} --hidden_dim {hidden_dim}"
                )
                return [
                    "--lr",
                    str(lr),
        ...
        print(
            f"ℹ️ Tuning ledger missing at {csv_path}. Falling back to standard profiles."
        )
        return default_args
    ...
    try:
        ...
        if valid.empty:
            print(
                f"⚠️ Tuning file {csv_path} contains no successful sweeps. Falling back to defaults."
            )
            return default_args
        ...
        if valid.empty:
            print(
                f"⚠️ No valid numerical optimization scores for {rnn_type or 'all'}. Falling back to defaults."
            )
            return default_args
        ...
        print(
            f"🎯 Optimization Checkpoint Found! Applying Tuned Parameters: --lr {lr} --dropout {dropout} --emb_dim {emb_dim} --hidden_dim {hidden_dim}"
        )
        return [
            "--lr",
            str(lr),
        ...
        print(
            f"⚠️ Hyperparameter parser interception exception: {e}. Default parameter profile applied."
        )
        return default_args
...
def get_best_empirical_settings(token_type):
    ...
    try:
        ...
        def get_composite_score(node):
            ...
            meteor = float(
                metrics.get("mean_meteor", node.get("meteor", 0.0))
            )
...
def execute_preprocessing(token_type="word", mock_mode=False):
    ...
    print(
        f"⚡ Running preprocessing routine (token_type={token_type}, mock={mock_mode})..."
    )
...
def run_automated_post_processing(token_type, rnn_type):
    ...
    if os.path.exists(de_en_model) and os.path.exists(en_sv_model):
        ...
        try:
            ...
            print(
                f"⚠️ Quantitative pivot dataset evaluation interrupted or unsupported: {e}"
            )
...
def execute_study_a(epochs, token_type, eval_queue: AsyncEvaluationQueue):
    ...
    for exp, cell, bidi in configs:
        ...
        if not is_cache_valid(
            os.path.join(OUTPUT_DIR, f"best_model_{exp_id}_{cell}.pt"),
            os.path.join(OUTPUT_DIR, f"best_config_{exp_id}_{cell}.json"),
            epochs,
        ):
            run_cmd(
                hparams
...
def execute_study_b(
    ...
    for exp, src, freeze, emb_dim in configs:
        ...
        if not is_cache_valid(
            os.path.join(OUTPUT_DIR, f"best_model_{exp_id}_{rnn_type}.pt"),
            os.path.join(OUTPUT_DIR, f"best_config_{exp_id}_{rnn_type}.json"),
            epochs,
        ):
            run_cmd(
                hparams
...
def execute_hyperparameter_tuning(
    stage, token_type, strategy, samples, epochs, extra_args=[]
):
    """
    Hyperparameter Tuning Sweep evaluated exclusively on Validation Loss (Val Loss).
    Bypasses metric evaluation (BLEU / METEOR) to maximize sweep throughput.
    """
    print(
    ...
    for idx, (lr, dropout, emb_dim, hidden_dim) in enumerate(selected_combos):
        ...
        try:
            ...
            expected_json = os.path.join(
                OUTPUT_DIR, f"best_config_{exp_tag}.json"
            )
            if not os.path.exists(expected_json):
                expected_json = os.path.join(
                    OUTPUT_DIR, f"best_config_{exp_id}.json"
                )
    ...
    if new_results:
        ...
        print(f"✅ [SUCCESS] Orchestrator tuning ledger updated -> {stage_csv}\n")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Empirically Cascaded NMT Pipeline Orchestrator"
    )
    parser.add_argument(
        "--study",
        type=str,
        required=True,
        choices=["A", "B", "C", "D", "E", "tune", "all"],
    )
    parser.add_argument(
        "--token_type",
        type=str,
    ...
    target_pathways = (
        ["word", "char"] if args.token_type == "both" else [args.token_type]
    )

    execute_preprocessing(
        token_type=getattr(args, "token_type", "word"), mock_mode=args.mock
    )
    ...
    try:
        if args.study == "tune":
            for pathway in target_pathways:
                ...
                epochs_coarse = (
                    max(1, int(runtime_epochs * 0.4 * char_factor))
                    if not args.mock
                    else 1
                )
            ...
            for pathway in target_pathways:
                char_factor = 1.25 if pathway == "char" else 1.0
                base_scale = (
                    max(1, int(runtime_epochs * char_factor))
                    if not args.mock
                    else 1
                )
                ...
                execute_study_a(
                    epochs_abc, token_type=pathway, eval_queue=eval_queue
                )
                ...
                run_automated_post_processing(
                    token_type=pathway, rnn_type=best["rnn_type"]
                )
            ...
            for pathway in target_pathways:
                ...
                if args.study == "A":
                    execute_study_a(
                        study_epochs, token_type=pathway, eval_queue=eval_queue
                    )
                elif args.study == "B":
                    execute_study_b(
                        study_epochs,
